...
REFERENCE = load_module("maize_reference_architecture", ROOT / "src/arabidopsis/models.py")
LATENT = load_module("maize_latent_architecture", ROOT / "src/wheat/model.py")
...
def protocol(args, ds):
    paths = [Path(__file__), ROOT / "src/maize/data.py", ROOT / "src/wheat/model.py", ROOT / "src/arabidopsis/models.py",
             ROOT / "data/maize/processed/height_observations.csv", ROOT / "data/maize/processed/weather_daily.csv"]
    return dict(split=args.split, train_years=ds.train_years, val_year=ds.val_year, test_year=ds.test_year,
                ...
                latent_profile="src/configs/wheat.json: small architecture, lr .01, physics 2, ymax .1",
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "outputs/maize-baselines")
