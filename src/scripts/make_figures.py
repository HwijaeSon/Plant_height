...
def main():
    ...
    temperature = load("submission_temperature_figures", ROOT / "src/visualization/temperature.py")
    temperature.OUT = args.output.resolve()
    temperature.wheat_examples()
    temperature.arabidopsis_examples()
    temperature.maize_examples()
    hypocotyl = load("submission_hypocotyl_figure", ROOT / "src/visualization/hypocotyl.py")
