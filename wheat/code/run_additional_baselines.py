...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "results/additional_baselines_seed1_3")
    args = parser.parse_args()
    out = args.output
    if (out / "all_runs.csv").exists():
        raise FileExistsError(f"Completed results already exist; choose a new --output: {out}")
    out.mkdir(parents=True, exist_ok=True)
    config_path = ROOT / "results/final_config.json"
    ...
    source_paths = [Path(__file__).resolve(), ROOT/"code/data.py", config_path, REPO/config["data"], REPO/config["kinship"]]
    ...
    protocol = dict(train_years=list(ds.train_years), val_year=ds.val_year, test_year=ds.test_year,
        ...
        implementation_reference="same CPU baseline family and fitting rules as maize/code/run_experiment.py, applied to wheat in metres",
