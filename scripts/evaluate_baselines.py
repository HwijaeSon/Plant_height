...
def main():
 ...
 torch.set_num_threads(2);sys.path.insert(0,str(ROOT/a.dataset/'code'))
 if a.dataset=='wheat' and a.model in ['lstm','pinn']:
  p.error('The wheat reference authors released predictions/summaries, not checkpoints in this release. See docs/REPRODUCTION.md.')
 if a.dataset=='arabidopsis' and a.model in ['lstm','pinn','rf']:
  p.error('Original Arabidopsis baseline weights were not saved. Predictions and retraining code are provided; see docs/REPRODUCTION.md.')
 ...
 if a.dataset in ['wheat','maize']:
  if a.dataset=='wheat':
   ...
   folder=ROOT/'wheat/results/additional_baselines_seed1_3';scale=1.
  else:
   import run_experiment as impl
   ds=impl.make_dataset(split='chronological');folder=ROOT/'maize/results/chronological_final_seed1_3';scale=ds.y_scale
  if a.model in ['logistic','temperature']:params=json.loads((folder/f'{a.model}_parameters.json').read_text())
  elif a.model=='rf':model=joblib.load(folder/f'runs/rf_seed{a.seed}/forest.joblib')
  else:
   params=json.loads((folder/'logistic_parameters.json').read_text());model=impl.build_model(a.model,ds,params)
   saved=torch.load(folder/f'runs/{a.model}_seed{a.seed}/checkpoint.pt',map_location='cpu',weights_only=False);model.load_state_dict(saved['state_dict']);model.eval()
  ...
  expected=np.load(folder/f'runs/{a.model}_seed{seed}/predictions.npz')
  ...
  params=json.loads((ROOT/'arabidopsis/results/fitted_process_parameters.json').read_text())[label]
  for split,attr in [('train','train_eval'),('val','val'),('test','test')]:
