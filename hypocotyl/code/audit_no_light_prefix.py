...
def main():
    ...
    parser.add_argument('--results', type=Path, default=ROOT/'hypocotyl/results/prefix_parameters_no_light_20260911')
    parser.add_argument('--output', type=Path, default=ROOT/'hypocotyl/reports/prefix_parameters_no_light_20260911/artifact_audit.json')
    ...
    assert len(search) == 30 and len(transfer) in [6,12]
    for path in search+transfer:
        run = json.loads(path.read_text())
        cfg = run['config']
        assert cfg['lambda_k'] == 0 and cfg['n_params'] == 1183
    ...
    assert final_plan['coefficient_variants'] == dict(selected=chosen['lambda_ode'], fixed100=100.)
    unique = set(final_plan['coefficient_variants'].values())
    assert len(transfer) == len(unique)*6 and len(final_plan['jobs']) == len(unique)*9
    assert len({job['output'] for job in final_plan['jobs']}) == len(final_plan['jobs'])
    assert selection['plan_sha256'] == sha(args.results/'plan.json')
    assert final_plan['lambda_selection_sha256'] == sha(args.results/'lambda_selection.json')
    selected = []
    for job in final_plan['jobs']:
        folder = args.results/job['output']
        run = json.loads((folder/'result.json').read_text())
        cfg = run['config']
        assert cfg['lambda_ode'] == job['lambda_ode'] and run['test_evaluations']==1
        assert sha(args.results/job['training_path']/'checkpoint.pt') == job['checkpoint_sha256']
        ...
        changed['env'] = torch.randn_like(changed['env'])*1234.
        with torch.no_grad():
            after=model(**changed)
        for field in ['pred','r','K']:
            torch.testing.assert_close(result[field],after[field],rtol=0,atol=0)
        torch.testing.assert_close(model.derivative(model(**x),x),
                                   model.derivative(model(**changed),changed),rtol=0,atol=0)
        selected.append(dict(path=job['output'], illumination_invariant=True, checkpoint_max_abs_mm=difference,
    ...
    report=dict(search_fits=30,transfer_fits=len(transfer),selected_test_evaluations=len(selected),
