"""Train no-light individual-parameter PhytoODE without loading or scoring test targets."""
import argparse
import hashlib
import json
import os
...
def initialize(seed, device):
    ...
    model = NoLightPrefixParameterPhytoODE().to(device)
...
def evaluate_checkpoint(checkpoint, output, device='cpu', selection_record=None):
    ...
    cfg = saved['config']
    if cfg['model'] != 'phytoode_prefix_parameters_no_light' or cfg['lambda_k'] != 0:
