...
import time
import zipfile
from run_single_condition import ROOT, OUT
...
def main():
    ...
    while not (OUT/'completed.json').exists():
        ...
        time.sleep(5)
    for path in ['hypocotyl/code/summarize_single_condition.py','paper/write_hypocotyl_results.py',
    ...
    assert 'alimchandani2026atlas' in source
    assert 'illumination plus duty cycle' not in source
    assert 'split-specific replicate means' in source
    ...
    pages=text.split('\f')
    record=dict(status='compiled_and_text_checks_passed',target_type='split_specific_replicate_means_12L12D_only',
        pdf_sha256=digest(paper/'main.pdf'),source_sha256=digest(paper/'main_standalone.tex'),
        figure_sha256=digest(paper/'figures/hypocotyl_single_condition.pdf'),
        pages=len(pages)-(not pages[-1].strip()),
        relevant_pages=[i+1 for i,t in enumerate(pages) if any(s in t for s in
            ['Hypocotyl growth: sparse','12L12D-only replicate-group holdout:',
             'Hypocotyl growth predictions for Col-0'])],
        references_resolved=True,overleaf_bundle_verified=True,visual_review_completed=False,
        manuscript_uses_split_specific_mean_metrics=True,environmental_input_features=0,
        cR_excluded_from_current_analysis=True,
        partition_provenance_retained_in_methods=True,
        latex_warnings=list(dict.fromkeys(s for s in result.stdout.splitlines() if 'warning:' in s.lower())))
