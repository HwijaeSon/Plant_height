...
PROFILE = ROOT / 'configs/hypocotyl_prefix_parameters_no_light_20260911.json'
...
def run_jobs(jobs, output, gpus, workers):
    ...
    def worker(gpu):
        while True:
            ...
            command = [sys.executable, str(ROOT / 'hypocotyl/code/no_light_prefix_trial.py'), 'train',
...
def main():
    ...
    p.add_argument('--output', type=Path, default=ROOT / 'hypocotyl/results/prefix_parameters_no_light_20260911')
    args = p.parse_args()
    args.output = args.output.resolve()
    args.output.mkdir(parents=True, exist_ok=True)
    profile = json.loads(PROFILE.read_text())
    sources = ['configs/hypocotyl_prefix_parameters_no_light_20260911.json',
        'hypocotyl/code/prefix_parameter_model.py', 'hypocotyl/code/no_light_prefix_model.py', 'hypocotyl/code/no_light_prefix_trial.py',
        'hypocotyl/code/run_no_light_prefix_search.py', 'hypocotyl/code/forecast_data.py',
    ...
    plan = dict(profile=profile, source_sha256={name: sha(ROOT/name) for name in sources},
                search_jobs=jobs, n_search_fits=len(jobs), n_transfer_fits_max=12,
    ...
    coefficient_variants = dict(selected=chosen, fixed100=profile['fixed_lambda_reference'])
    unique_coefficients = sorted(set(coefficient_variants.values()))
    transfer = [dict(lambda_ode=value, seed=seed, drop_fraction=drop,
                     path=f'transfer/lambda_{lambda_tag(value)}/drop_{round(drop*100)}/seed{seed}')
                for value in unique_coefficients for drop in [.25, .5] for seed in profile['seeds']]
    run_jobs(transfer, args.output, args.gpus, args.workers_per_gpu)
    test_jobs = []
    for value in unique_coefficients:
        for drop in profile['evaluation_drop_fractions']:
            for seed in profile['seeds']:
                source = (f'search/lambda_{lambda_tag(value)}/seed{seed}' if drop == 0
                          else f'transfer/lambda_{lambda_tag(value)}/drop_{round(drop*100)}/seed{seed}')
                ...
                test_jobs.append(dict(lambda_ode=value, drop_fraction=drop, seed=seed, training_path=source,
                    checkpoint_sha256=sha(checkpoint), output=f'evaluated/lambda_{lambda_tag(value)}/drop_{round(drop*100)}/seed{seed}'))
    final_plan = dict(lambda_selection_sha256=sha(args.output/'lambda_selection.json'), jobs=test_jobs,
        coefficient_variants=coefficient_variants,
        note='The validation-selected and fixed-reference coefficients and all unique checkpoint hashes are frozen before test scoring.')
    ...
    frozen(args.output/'completion.json', dict(search_fits=len(jobs), transfer_fits=len(transfer),
        selected_test_evaluations=len(test_jobs), selected_lambda_ode=chosen, fixed_lambda_reference=profile['fixed_lambda_reference'], lambda_k=0., failures=[]))
