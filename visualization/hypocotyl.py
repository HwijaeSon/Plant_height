...
HERE = ROOT / 'outputs'
RELEASE = ROOT
PREPARED = ROOT / 'hypocotyl/data/processed/four_genotypes_20260915'
REFERENCE = ROOT / 'hypocotyl/results/four_genotypes_20260915'
...
def run():
    ...
    for model in ORDER:
        ...
        for seed in seeds:
            if model in ORDER[:4]:
                folder = REFERENCE / 'drop_0' / model / f'seed{seed}'
            elif model == 'phytoode2':
                folder = ROOT / 'hypocotyl/results/manuscript/phytoode/drop_0' / f'seed{seed}'
            else:
                folder = ROOT / 'hypocotyl/results/manuscript/latent_ode/drop_0' / f'seed{seed}'
            result = json.loads((folder / 'result.json').read_text())
            arrays = np.load(folder / 'predictions.npz')
